...
public static class MessageExtensions
{
    ...
    #region PlayerStates
    public static Message AddPlayerStates(this Message message, Dictionary<ushort, PlayerState> value) => Add(message, value);

    public static Message Add(this Message message, Dictionary<ushort, PlayerState> value)
    {
        message.AddUShort((ushort)value.Keys.Count());

        foreach (var playerState in value)
        {
            message.AddUShort(playerState.Key);
            message.AddString(playerState.Value.Name);
            message.AddBool(playerState.Value.IsLoading);
        }

        return message;
    }

    public static Dictionary<ushort, PlayerState> GetPlayerStates(this Message message)
    {
        ushort count = message.GetUShort();
        Dictionary<ushort, PlayerState> states = new Dictionary<ushort, PlayerState>();

        for (int i = 0;  i < count; i++)
        {
            ushort id = message.GetUShort();
            states.Add(id, new PlayerState() { Id = id, Name = message.GetString(), IsLoading = message.GetBool() } );
        }
